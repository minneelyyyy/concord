                            ...
                            struct discord_ret_user *ret)
{
    ...
    CCORD_EXPECT(
        client,
        discord_modify_current_user_to_json(&body.start, &body.size, params)
            == JSONB_OK,
                  ...
                  struct discord_ret_channel *ret)
{
    ...
    CCORD_EXPECT(client, params != NULL, CCORD_BAD_PARAMETER, "");
    CCORD_EXPECT(client,
                 discord_create_dm_to_json(&body.start, &body.size, params)
                     == JSONB_OK,
                 CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
                        ...
                        struct discord_ret_channel *ret)
{
    ...
    CCORD_EXPECT(
        client,
        discord_create_group_dm_to_json(&body.start, &body.size, params)
            == JSONB_OK,
        CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
    DISCORD_ATTR_INIT(attr, discord_channel, ret, NULL);
    return discord_rest_run(&client->rest, &attr, &body, HTTP_POST,
