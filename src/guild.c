                     ...
                     struct discord_ret_guild *ret)
{
    ...
    CCORD_EXPECT(client,
                 discord_create_guild_to_json(&body.start, &body.size, params)
                     == JSONB_OK,
                 CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
                     ...
                     struct discord_ret_guild *ret)
{
    ...
    CCORD_EXPECT(client, params != NULL, CCORD_BAD_PARAMETER, "");
    CCORD_EXPECT(client,
                 discord_modify_guild_to_json(&body.start, &body.size, params)
                     == JSONB_OK,
                 CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
                             ...
                             struct discord_ret_channel *ret)
{
    ...
    CCORD_EXPECT(
        client,
        discord_create_guild_channel_to_json(&body.start, &body.size, params)
            == JSONB_OK,
        CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
                         ...
                         struct discord_ret_guild_member *ret)
{
    ...
    CCORD_EXPECT(
        client,
        discord_add_guild_member_to_json(&body.start, &body.size, params)
            == JSONB_OK,
        CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
    DISCORD_ATTR_INIT(attr, discord_guild_member, ret, NULL);
                            ...
                            struct discord_ret_guild_member *ret)
{
    ...
    CCORD_EXPECT(
        client,
        discord_modify_guild_member_to_json(&body.start, &body.size, params)
            == JSONB_OK,
        CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
                              ...
                              struct discord_ret_guild_member *ret)
{
    ...
    CCORD_EXPECT(
        client,
        discord_modify_current_member_to_json(&body.start, &body.size, params)
            == JSONB_OK,
        CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
    DISCORD_ATTR_INIT(attr, discord_guild_member, ret, params->reason);
    return discord_rest_run(&client->rest, &attr, &body, HTTP_PATCH,
                         ...
                         struct discord_ret *ret)
{
    ...
    CCORD_EXPECT(
        client,
        discord_create_guild_ban_to_json(&body.start, &body.size, params)
            == JSONB_OK,
        CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
                          ...
                          struct discord_ret_role *ret)
{
    ...
    CCORD_EXPECT(
        client,
        discord_create_guild_role_to_json(&body.start, &body.size, params)
            == JSONB_OK,
        CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
                          ...
                          struct discord_ret_role *ret)
{
    ...
    CCORD_EXPECT(
        client,
        discord_modify_guild_role_to_json(&body.start, &body.size, params)
            == JSONB_OK,
        CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
    DISCORD_ATTR_INIT(attr, discord_role, ret, params ? params->reason : NULL);
    return discord_rest_run(&client->rest, &attr, &body, HTTP_PATCH,
                          ...
                          struct discord_ret *ret)
{
    ...
    CCORD_EXPECT(
        client,
        discord_begin_guild_prune_to_json(&body.start, &body.size, params)
            == JSONB_OK,
        CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
    DISCORD_ATTR_BLANK_INIT(attr, ret, params ? params->reason : NULL);
    return discord_rest_run(&client->rest, &attr, &body, HTTP_POST,
                            ...
                            struct discord_ret_guild_widget_settings *ret)
{
    ...
    CCORD_EXPECT(
        client,
        discord_guild_widget_settings_to_json(&body.start, &body.size, params)
            == JSONB_OK,
        CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
                                ...
                                struct discord_ret *ret)
{
    ...
    CCORD_EXPECT(client, guild_id != 0, CCORD_BAD_PARAMETER, "");
    CCORD_EXPECT(client,
                 discord_modify_user_voice_state_to_json(&body.start,
                                                         &body.size, params)
                     == JSONB_OK,
                 CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
    DISCORD_ATTR_BLANK_INIT(attr, ret, NULL);
