                       ...
                       struct discord_ret_webhook *ret)
{
    ...
    CCORD_EXPECT(
        client,
        discord_create_webhook_to_json(&body.start, &body.size, params)
            == JSONB_OK,
                       ...
                       struct discord_ret_webhook *ret)
{
    ...
    CCORD_EXPECT(
        client,
        discord_modify_webhook_to_json(&body.start, &body.size, params)
            == JSONB_OK,
        CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
    ...
    struct discord_ret_webhook *ret)
{
    ...
    CCORD_EXPECT(client, NOT_EMPTY_STR(webhook_token), CCORD_BAD_PARAMETER,
                 "");
    CCORD_EXPECT(client,
                 discord_modify_webhook_with_token_to_json(&body.start,
                                                           &body.size, params)
                     == JSONB_OK,
                 CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
    DISCORD_ATTR_INIT(attr, discord_webhook, ret, NULL);
                        ...
                        struct discord_ret *ret)
{
    ...
    CCORD_EXPECT(
        client,
        discord_execute_webhook_to_json(&body.start, &body.size, params)
            == JSONB_OK,
        CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
    DISCORD_ATTR_BLANK_INIT(attr, ret, NULL);
                             ...
                             struct discord_ret_message *ret)
{
    ...
    CCORD_EXPECT(
        client,
        discord_edit_webhook_message_to_json(&body.start, &body.size, params)
            == JSONB_OK,
        CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
    DISCORD_ATTR_INIT(attr, discord_message, ret, NULL);
    if (params->attachments) {
