    ...
    struct discord_ret_interaction_response *ret)
{
    ...
    CCORD_EXPECT(
        client,
        discord_interaction_response_to_json(&body.start, &body.size, params)
            == JSONB_OK,
                                ...
                                struct discord_ret_webhook *ret)
{
    ...
    CCORD_EXPECT(client, params != NULL, CCORD_BAD_PARAMETER, "");
    CCORD_EXPECT(client,
                 discord_create_followup_message_to_json(&body.start,
                                                         &body.size, params)
                     == JSONB_OK,
                 CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
                              ...
                              struct discord_ret_message *ret)
{
    ...
    CCORD_EXPECT(
        client,
        discord_edit_followup_message_to_json(&body.start, &body.size, params)
            == JSONB_OK,
        CCORD_MALFORMED_PAYLOAD, "Failed to encode JSON");
    DISCORD_ATTR_INIT(attr, discord_message, ret, NULL);
    if (params->attachments) {
